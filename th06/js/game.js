...
function dollari(s, p) {
	return dollar.apply(null, arguments)[0];
}
function dollare(id) {
	return document.getElementById(id);
}
function dollarnew(t, d) {
...
function dollarattr(e, a, v) {
	if (same_type(e, '')) e = dollari(e);
	if (v === undefined) {
...
function dollarreaddClass(e, c) {
	e.classList.remove(c);
	// MAGIC!
	e.offsetWidth = e.offsetWidth;
	e.classList.add(c);
}
function dollaraddCssRule(s, r) {
...
var RES = (function(res) {
	...
	function getColorRGB(color) {
		...
		return dollarstyle(div, 'background-color').match(/\d+/g).map(parseFloat);
	}
	function updateCanvas(canv) {
		var dc = canv.getContext('2d'),
			sc = getSrc(dollarattr(canv, 'from')),
			ts = getTrans(dollarattr(canv, 'transform')),
			scale = getScale(dollarattr(canv, 'scale')),
			stack = getStack(dollarattr(canv, 'stack'));
	...
	function loaded() {
		ieach(res.children, function(i, v, d) {
			if (v.tagName == 'CANVAS') {
				if (dollarattr(v, 'from'))
					updateCanvas(v);
				d[v.id] = v;
			}
			else if (v.tagName == 'AUDIO') {
				//if (!v.src && dollarattr(v, 'audio-src'))
				//	v.src = dollarattr(v, 'audio-src');
				v.replayTime = parseFloat(dollarattr(v, 'replay-time') || '0');
				if ((v.replayQueueLength = parseFloat(dollarattr(v, 'replay-queue'))) > 0) {
					var queue = [v],
						index = 0;;
			...
			else if (v.tagName == 'SCRIPT' && dollarattr(v, 'type') == 'text/html') {
				d[v.id] = v.innerHTML;
			}
	...
	_t.check = function check(fn, fe) {
		var ls = ieach(res.children, function(i, v, d) {
			if (v.tagName == 'IMG')
				...
				d.push(v.complete || dollarattr(v, 'complete') ? 1 : 0);
		}, []);
...
GAME.init = function() {
	...
	var uiTick = newTicker(100, function(cbs) {
		...
		['ui-bind', function(e) {
			return Function('val', dollarattr(e, 'ui-bind-exec') || ((dollarattr(e, 'ui-bind-attr') || 'this.innerHTML')+'=val'));
		}],
		['ui-show', function(e) {
			return function(v) {
...
function newBomb(player) {
	...
	if (bg.active = dollari('.sc-bomb')) {
		bg.scelem = bg.active.parentNode;
		dollarreaddClass(bg.scelem, 'active');
		dollari('.text', bg.scelem).innerHTML = RES.st_bomb_sc;
	}
...
function newBossBackground(boss, elem) {
	...
	elem.object = obj;
	elem.speed = parseFloat(dollarattr(elem, 'bg-speed') || 0);
	elem.rotate = parseFloat(dollarattr(elem, 'bg-rotate') || 0);
	if (location.search != '?nobg') obj.anim(50, function(d) {
		elem.style.opacity = d.ph;
		if (elem.speed)
			elem.style.backgroundPosition = '0 '+(-d.age*elem.speed)+'px';
		if (elem.rotate) {
			dollarstyle(elem, 'transform-origin', '50% 50%');
			dollarstyle(elem, 'transform', 'rotate('+(-d.age*elem.rotate)+'deg)');
		}
	}, obj.data)
	return obj;
}
...
function newBackground(scrollImgs) {
	...
	ieach(scrollImgs, function(i, e) {
		e.object = bg;
		e.trans = dollarattr(e, 'bg-trans') || '';
		e.offsetX = parseFloat(dollarattr(e, 'bg-offset-x') || '0');
		e.offsetY = parseFloat(dollarattr(e, 'bg-offset') || '0');
		e.offsetZ = parseFloat(dollarattr(e, 'bg-offset-z') || '0');
		e.totalX = parseFloat(dollarstyle(e, 'width')) - parseFloat(dollarattr(e, 'bg-repeat-x') || dollarstyle('.game', 'width'));
		e.totalY = parseFloat(dollarstyle(e, 'height')) - parseFloat(dollarattr(e, 'bg-repeat') || dollarstyle('.game', 'height'));
		e.totalZ = parseFloat(dollarstyle(e, 'height')) - parseFloat(dollarattr(e, 'bg-repeat-z') || dollarstyle('.game', 'height'));
		e.speedX = parseFloat(dollarattr(e, 'bg-speed-x') || '0');
		e.speedY = parseFloat(dollarattr(e, 'bg-speed') || '0');
		e.speedZ = parseFloat(dollarattr(e, 'bg-speed-z') || '0');
	});
	if (location.search == '?nobg') ieach(scrollImgs, function(i, e) {
		...
		ieach(scrollImgs, function(i, e) {
			if (e.speedX || e.speedY || e.speedZ) {
				...
				dollarstyle(e, 'transform', e.trans+' '+trans);
			}
		});
	});
	return bg;
...
function updateBgImg(e, f) {
	...
	dollarstyle(e, 'transform', 'perspective('+p+'px) rotateX('+r+'deg)');
	dollarstyle(e, 'transform-origin', '50% '+y+'px');
...
function newStgSecDiag(next, para) {
	return {
		init: function(d) {
			...
			var bg = dollari('.diag'),
				pos = dollari(para.pos, bg),
				face = dollari(para.face, pos),
				text = dollari('.ft.dg .text', bg);
			var filter = function(i, e, d) {
				e === d ? e.classList.add('active') : e.classList.remove('active');
			};
			bg.classList.add('active');
			ieach(dollar('.dg', bg), filter, pos);
			ieach(para.clear ? dollar('.dg .face', bg) : dollar('.face', pos), filter, face);
...
function newStgSecBoss(next, para) {
	...
	return {
		init: function(d) {
			...
			if (para.scname) {
				if (d.scactive = dollare(para.scface || 'face_sc_'+d.boss.data.boss)) {
					d.scelem = d.scactive.parentNode;
					dollarreaddClass(d.scelem, 'active');
					d.txtelem = dollari('.text', d.scelem);
					d.txtelem.innerHTML = para.scname;
					setTimeout(function() {
						var x = Math.min(128-d.txtelem.scrollWidth, 0);
						dollarstyle(d.txtelem, 'transform', 'translateX('+x+'px)');
					}, 50);
					ieach(dollar('.face', d.scelem), function(i, e) {
						e.classList[e === d.scactive ? 'remove' : 'add']('hidden');
					});
				}
				if (d.bgelem = dollare(para.background || 'bg_sc_'+d.boss.data.boss)) STORY.timeout(function() {
...
function newStgSecAskContinue(next, para) {
	return {
		init: function(d) {
			...
			dollari('.game.pause').insertBefore(dollarnew('div', {
				id: 'pause_notice',
				style: 'position:absolute;margin:1em',
				innerHTML: RES.st_ask_continue,
			}), dollari('.menu-pause-text'));
		},
		run: function(dt, d) {
	...
	return {
		init: function(d) {
			...
			d.mask.anim(50, function(d) {
				...
				dollarstyle(elem, 'transform', 'translateY(' + offset + 'px)');
			}, d.mask.data);
		},
		run: function(dt, d) {
			d.age = (d.age || 0) + dt;
