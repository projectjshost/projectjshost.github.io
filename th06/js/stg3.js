function newStg3BgAnim(bg) {
	var e = dollare('bg_stg3'),
		gnd = dollare('stg3cv'),
		mask = dollare('bg_stg3_mask');
	e.width = parseFloat(dollarstyle(e, 'width'));
	e.height = parseFloat(dollarstyle(e, 'height'));
	e.val = {
		persp: 600,
		rotate: [50, 30],
	}
	...
	var cv = dollare('bg_stg3_dot'),
		dc = cv.getContext('2d');
	cv.width = parseFloat(dollarstyle(cv, 'width'));
	cv.height = parseFloat(dollarstyle(cv, 'height'));
	bg.anim(50, function(d) {
