...
function checkActiveRadios() {
	var ls = ieach(dollar('input[type="radio"]'), function(i, v, d) {
		if (v.scrollHeight)
			d.push(v);
	}, []);
...
function evalRadioVal(name) {
	var val = getRadioVal(dollar('input[name="'+name+'"]'));
	if (val && val != 'on') {
		eval(val);
		RES.se_ok00.replay();
	}
...
function isPracticeMode() {
	return !dollar('.menu-title input:checked + .menu-title-start')[0]
}

ieach(dollar('form[eval-radios]'), function(i, e) {
	...
	ieach(dollar('input[type="radio"]', e), function(j, r) {
		r.name = 'eval_radio'+i;
	});
});
ieach(dollar('input[type="radio"]'), function(i, v) {
...
ieach(dollar('.char-string[char-string]'), function(i, e) {
	ieach(dollarattr(e, 'char-string'), function(i, c) {
		e.appendChild(dollarnew('span', {
			className: 'char char-'+c,
		}))
	})
});
ieach(dollar('.ui-obj'), function(i, v) {
	v.className += ' ui';
...
RES.check(function() {
	...
	ieach(dollar('.show-game-error'), function(i, e) {
...
ieach(dollar('.show-on-tested-browser, .hide-on-tested-browser'), function(i, e) {
