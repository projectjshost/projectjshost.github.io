...
function loadTheme(theme) {
        switch(theme) {
            case "light":
                $("#customTheme").remove();
                $("#JStheme").remove();
                $('head').append('<link rel="stylesheet" id="JStheme" href="/jsappapi/2.3.1/themes/light.css">');
                break;
            case "dark":
                $("#customTheme").remove();
                $("#JStheme").remove();
                $('head').append('<link rel="stylesheet" id="JStheme" href="/jsappapi/2.3.1/themes/dark.css">');
                break;
            case "custom":
                $("#customTheme").remove();
                $("#JStheme").remove();
...
$(document).ready(function(){
    if (!localStorage.theme||localStorage.lastUsedVersion!=="2.3.1") {
    ...
    if (document.getElementById("header")) {
        ...
        $("#header").append("<div class='headerButtons'><button class='headerButton icon' onclick='dialog(projectJS.versionFull)'>info</button><button class='headerButton icon' id='launcherButton' onclick='location.replace(projectJS.launcher)'>home</button></div>");
    }
...
function switchInterface(name) {
        switch (name) {
            case "classic":
                $("#JSinterface").remove();
                $("#JSaccent").remove();
                $('head').append('<link rel="stylesheet" id="JSinterface" href="/jsappapi/2.3.1/interfaces/classic.css">');
                $('head').append('<style id="JSaccent">:root{--accent:#666;--accent2:#666}</style>');
                break;
            case "modern":
                $("#JSinterface").remove();
                $("#JSaccent").remove();
                $('head').append('<link rel="stylesheet" id="JSinterface" href="/jsappapi/2.3.1/interfaces/modern.css">');
            ...
            case "material":
                $("#JSinterface").remove();
                $("#JSaccent").remove();
                $('head').append('<link rel="stylesheet" id="JSinterface" href="/jsappapi/2.3.1/interfaces/material.css">');
                if (typeof accentColor == 'undefined') {
                    accentColor = "#76A7E0";
                }
