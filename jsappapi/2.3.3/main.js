...
const defaultWallpaperName = "mountain-landscape.jpg";
const defaultWallpaper = `/lib/wallpapers/mountain-landscape.jpg`;
var wallpaperCreated=false;
// Sets Wallpaper of App
function createWallpaper(source) {
    if(wallpaperCreated==false) {
...
function loadTheme(theme) {
    switch (theme) {
        case "light":
            $("#customTheme").remove();
            $("#JStheme").remove();
            $('head').append('<link onload="getAppTileColor()" rel="stylesheet" id="JStheme" href="/jsappapi/2.3.3/themes/light.css">');
            break;
        case "dark":
            $("#customTheme").remove();
            $("#JStheme").remove();
            $('head').append('<link onload="getAppTileColor()" rel="stylesheet" id="JStheme" href="/jsappapi/2.3.3/themes/dark.css">');
            break;
        case "custom":
            $("#customTheme").remove();
            $("#JStheme").remove();
...
$(document).ready(function () {
    if (!localStorage.theme || localStorage.lastUsedVersion !== "2.3.3") {
    ...
    if (document.getElementById("header")) {
        ...
        $("body").append("<div class='headerButtons'><button class='headerButton icon' onclick='displayAppInfo()'>info</button><button class='headerButton icon' id='launcherButton' onclick='location.replace(projectJS.launcher)'>home</button></div>");
    }
...
function switchInterface(name) {
    switch (name) {
        case "classic":
            $("#JSinterface").remove();
            $("#JSaccent").remove();
            $('head').append('<link rel="stylesheet" id="JSinterface" href="/jsappapi/2.3.3/interfaces/classic.css">');
            $('head').append('<style id="JSaccent">:root{--accent:#666;--accent2:#666}</style>');
            break;
        case "modern":
            $("#JSinterface").remove();
            $("#JSaccent").remove();
            $('head').append('<link rel="stylesheet" id="JSinterface" href="/jsappapi/2.3.3/interfaces/modern.css">');
        ...
        case "material":
            $("#JSinterface").remove();
            $("#JSaccent").remove();
            $('head').append('<link rel="stylesheet" id="JSinterface" href="/jsappapi/2.3.3/interfaces/material.css">');
            if (typeof accentColor == 'undefined') {
                accentColor = "#76A7E0";
            }
